...
if($num_args eq 1) {
	...
	print(STDERR "Arguments needed: full filename of PDF file.\n");
	exit 1;
}
...
my $result = system("git pull --rebase origin master ; git add -u ; git commit -m \"Modifies $pdf after last session\" ; git push origin master");
if(!$result) {
